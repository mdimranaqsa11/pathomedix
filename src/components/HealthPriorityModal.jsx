...
export default function HealthPriorityModal() {
  ...
  // Check device type and auto open message modal
  useEffect(() => {
    // Check if mobile
    const checkMobile = () => {
  ...
  const handleMessageOk = () => {
    ...
    setTimeout(() => {
      ...
      // Open main modal after message modal closes
      setTimeout(() => {
        setOpen(true);
        setIsAnimating(true);
      }, 300);
        ...
        {showMessageModal && (
          <>
            ...
            <motion.div
              ...
              aria-hidden="true"
              initial={{ opacity: 0 }}
              animate={{ opacity: messageAnimating ? 1 : 0 }}
              transition={{ duration: 0.4, ease: "easeOut" }}
              style={{
                backgroundColor: "rgba(0, 0, 0, 0.55)",
                backdropFilter: messageAnimating ? "blur(5px)" : "blur(0px)",
              }}
              exit={{ opacity: 0 }}
            />
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-6">
              ...
              >
                <div className="relative overflow-hidden rounded-3xl bg-white shadow-2xl p-6 sm:p-8">
                  {/* Close Button */}
                  <motion.button
                    onClick={handleCloseMessage}
                    className="absolute right-3 sm:right-4 top-3 sm:top-4 flex h-9 sm:h-10 w-9 sm:w-10 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow-md backdrop-blur hover:scale-110 active:scale-95 transition-transform"
                    initial={{ scale: 0 }}
                    animate={messageAnimating ? { scale: 1 } : { scale: 0 }}
                  ...
                  <div className="text-center space-y-4">
                    ...
                    <motion.div
                      ...
                      animate={messageAnimating ? { scale: 1, rotate: 0 } : {}}
                      transition={{ delay: 0.2, duration: 0.5 }}
                    >
                      <div className="flex h-16 sm:h-20 w-16 sm:w-20 items-center justify-center rounded-full bg-gradient-to-br from-[#6f5cff] via-[#7c68eb] to-[#927fff]">
                        <MessageCircle
                          size={isMobile ? 32 : 40}
                    ...
                    <motion.div
                      ...
                      animate={messageAnimating ? { opacity: 1, y: 0 } : {}}
                      transition={{ delay: 0.3 }}
                    >
                      ...
                      <p className="text-sm sm:text-base text-gray-600 leading-relaxed">
                        Not just doctors, Super Speciality care you can trust
                      </p>
                    </motion.div>
                  </div>
                  ...
                  <motion.button
                    ...
                    className="w-full mt-6 sm:mt-8 rounded-xl bg-gradient-to-r from-[#6f5cff] to-[#927fff] px-4 py-3 sm:py-4 text-sm sm:text-base font-semibold text-white transition-all hover:shadow-lg active:scale-95"
                    initial={{ opacity: 0, y: 10 }}
                    animate={messageAnimating ? { opacity: 1, y: 0 } : {}}
